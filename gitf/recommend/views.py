...
def getOnePost(request):
    if request.method == "POST":
        ...
        Comments = Comment.objects.filter(postID=post['id'])
        result = []
        ...
        for comment in Comments.values():
            ...
            result.append(this)
        this_post['comments'] = result

        
        data = json.dumps(this_post, ensure_ascii = False)
        return HttpResponse(data)
    else:
        return HttpResponse("에러")
...
def writeComment(request):
    if request.method == "POST":
        ...
        Comment(postID=Post.objects.get(pk=postId), content=content).save()
        return HttpResponse("OK")
    else:
        return HttpResponse("not Post")
...
#             product['name'] = product_object['name']
#             product['desc'] = product_object['desc']
#             product['image'] = product_object['image']
#             product['common_price'] = product_object['common_price']
#             comment = []
#             Post_qs = Post.objects.filter(tags__icontains=product['name'])
#             for j in Post_qs.values():      ## 상품명이 태그로 들어간 글들에서
...
#             products.append(product)
#         data = json.dumps(products, ensure_ascii = False)
#         return render(request, 'recommend_result.html', {'products': products})
#     else:
#         return HttpResponse("not post")
